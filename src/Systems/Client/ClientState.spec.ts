...
describe('Client State', function() {
  ...
  it('should assign state data when passed in', function() {
    clientState = ClientState.create(
      new ClientStateData(
        ApplicationToken.create('test', '1'),
        new ApplicationClient('test', 'test')
      )
    );
    expect(
      clientState.stateData.token.name === 'test' &&
        clientState.stateData.client.name === 'test'
    ).toBeTruthy();
  });
  it('should assign state data should reset', function() {
    ...
    expect(
      clientState.stateData.token.name === '' &&
        clientState.stateData.client.name === ''
    ).toBeTruthy();
  });
});
